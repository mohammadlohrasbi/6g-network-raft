...
function loadContracts() {
  ...
  entry.contracts.forEach((t) => {
    ...
    o.textContent = t.contract + (t.writable ? '' : ' — read only')
      + (t.antennaDep ? ' — needs an antenna record' : '');
    o.dataset.writable = String(t.writable);
    o.dataset.antennaDep = String(t.antennaDep);
    o.dataset.fn = t.fn || '';
    sel.appendChild(o);
  });
  describeContract();
...
function describeContract() {
  ...
  } else if (opt.dataset.antennaDep === 'true') {
    hint.textContent = 'Reads an antenna record before writing — seed one or it will not commit';
  } else {
...
function buildChannelPicker() {
  ...
  state.catalog.channels.forEach((c) => {
    const runnable = c.contracts.filter((t) => t.writable && !t.antennaDep).length;
